    ...
    <div className="group relative flex flex-col w-full h-[400px] rounded-[30px] bg-[#795050] border border-white/5 overflow-hidden transition-all duration-700 hover:border-[#e63c3c]/50 hover:shadow-[0_0_40px_rgba(230,60,60,0.2)]">
      ...
      <div className="relative w-full h-40 overflow-hidden">
        <img src={image || "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=80&w=500"} 
             className="w-full h-full object-cover opacity-40 group-hover:opacity-100 group-hover:scale-110 transition-all duration-1000" alt={title} />
        {popular && <span className="absolute top-4 right-4 px-3 py-1 bg-[#00f2ea]/20 text-[#00f2ea] text-[9px] font-bold uppercase rounded-full border border-[#00f2ea]/30 z-20">Popular</span>}
        <div className="absolute inset-0 bg-gradient-to-t from-[#111111] via-transparent to-transparent" />
      </div>

      {/* Content Section */}
      <div className="p-7 flex flex-col flex-grow relative">
        <span className="absolute -top-4 right-8 text-7xl font-black text-white/5 group-hover:text-[#e63c3c]/10 transition-all duration-700 italic">{id}</span>
        ...
        <h3 className="text-xl font-bold text-white mb-2 italic group-hover:text-[#e63c3c] transition-colors uppercase">{title}</h3>
        <p className="text-gray-400 text-[11px] leading-relaxed mb-4 line-clamp-2">{desc}</p>

        <div className="flex flex-wrap gap-2 mt-auto mb-6">
          {tags.map((tag, i) => (
            <span key={i} className="text-[9px] font-bold text-zinc-500 px-3 py-1 bg-white/5 rounded-md border border-white/5 uppercase">✓ {tag}</span>
          ))}
        </div>

        <div className="flex items-center gap-2 text-white text-[10px] font-black uppercase tracking-widest group/btn">
          Discover More <span className="text-[#e63c3c] group-hover/btn:translate-x-2 transition-transform">→</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="max-w-[95%] mx-auto relative">
        ...
        <div className="mb-16 text-left group">
          <h2 className="text-white text-5xl md:text-6xl font-black uppercase italic tracking-tighter transition-all duration-700 hover:tracking-normal cursor-default">
            Our <span className="text-[#e63c3c] relative inline-block">
              Services
            </span>
          </h2>
        ...
        {/* 2. Feature Section */}
        <div className="flex flex-col lg:flex-row items-center gap-12 mb-32 reveal">
          <div className="w-full lg:w-1/2 h-[450px] rounded-[40px] overflow-hidden border border-white/10 shadow-[0_20px_50px_rgba(230,60,60,0.1)] reveal reveal-left">
            <img 
          ...
          <div className="w-full lg:w-1/2 text-left space-y-6 reveal reveal-right">
            ...
            <p className="text-gray-400 text-sm md:text-base leading-relaxed max-w-xl font-medium">
              We don't just build websites; we engineer digital experiences that dominate the market. Our approach combines brutalist design aesthetics with cutting-edge performance to ensure your brand stands out in the noise.
            </p>
            <div className="flex items-center gap-4 pt-4">
               <div className="w-12 h-[1px] bg-[#e63c3c]"></div>
               <span className="text-[#e63c3c] text-[10px] font-black uppercase tracking-[4px]">Elite Agency Standard</span>
            </div>
          </div>
        </div>

        {/* 3. Our Projects Heading (Red Line Removed) */}
        <div id="projects" className="mb-16 text-left group reveal reveal-left">
          <h2 className="text-white text-5xl md:text-6xl font-black uppercase italic tracking-tighter transition-all duration-700 hover:tracking-normal cursor-default">
            Our <span className="text-[#e63c3c] relative inline-block">
              Projects
            </span>
        ...
        {/* 4. Main Grid Structure (Project Cards) */}
        <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-8 items-stretch reveal reveal-bottom">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <ServiceCard id="01" popular={true} title="Visual Branding" desc="Tailored visual identities that speak your brand's language." tags={["Logo", "Strategy"]} />
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="group relative flex-grow p-10 rounded-[30px] bg-gradient-to-br from-[#e63c3c] to-[#500000] flex flex-col items-center justify-center text-center overflow-hidden min-h-[300px]">
              ...
              <h3 className="text-3xl font-black text-white italic uppercase leading-none">Transform <br/> Vision</h3>
              <button className="mt-8 w-full py-4 bg-white text-black font-black rounded-2xl text-[11px] uppercase tracking-widest hover:bg-black hover:text-white transition-all shadow-xl active:scale-95">Launch Project →</button>
            </div>
          </div>
        </div>
